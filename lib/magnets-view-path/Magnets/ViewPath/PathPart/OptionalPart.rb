...
module Rmagnets::ViewPath::PathPart::OptionalPart
	...
	# [ optional_part1_1, optional_part1_2, [ optional_part2_4 ], optional_part1_3 ]
	def match_request( remaining_descriptor_elements, remaining_request_path_parts )
		...
		number_non_optional_descriptors_remaining	=	remaining_descriptor_elements.select do |element|
		...
		nested_permutations.each do |this_optional_set|
			...
			# and make sure we have enough path parts to accomodate remaining 
			# descriptors if this set matches
			next unless path_accomodates_optional_set( number_non_optional_descriptors_remaining, 
			                                           remaining_request_path_parts, 
			                                           this_optional_set )
			
			# * we look ahead the # of possible optional parts and attempt to match n + 1 until the end
			next unless match_remaining_descriptors_for_optional_set( this_optional_set, 
			                                                          remaining_descriptor_elements.dup, 
			                                                          remaining_request_path_parts )
			
			# * if it matches (including no further descriptor/path parts), 
			#   attempt to match each option descriptor in this set
			if matched_paths = match_optional_set( this_optional_set, 
			                                       remaining_descriptor_elements.dup, 
			                                       remaining_request_path_parts )
	...
	def path_parts_remaining_after_optional_set( optional_set, remaining_request_path_parts )
		...
		return remaining_request_path_parts.slice( optional_set.count, 
		                                           remaining_request_path_parts.count )
	...
	def match_remaining_descriptors_for_optional_set( optional_set, 
	                                                  remaining_descriptor_elements, 
	                                                  remaining_request_path_parts )

		matched_paths = nil

		remaining_path_parts = path_parts_remaining_after_optional_set( optional_set, 
		                                                                remaining_request_path_parts )
		...
		unless remaining_path_parts.nil?
			...
			until remaining_descriptor_elements.empty?
				...
				# if one of our descriptors returns failure for matching, 
				# this set does not match - go to the next set
				break unless this_remaining_descriptor.match_request( remaining_descriptor_elements, 
	...
	#####################################
	#  number_remaining_parts_if_match  #
	#####################################
	
	def number_remaining_parts_if_match( remaining_request_path_parts, 
	                                     optional_set )
	...
	def path_accomodates_optional_set( number_non_optional_descriptors_remaining, 
	                                   remaining_request_path_parts, 
	                                   optional_set )
		...
		if remaining_request_path_parts.count >= optional_set.count	      and
			 number_remaining_parts_if_match( remaining_request_path_parts, 
			                                  optional_set ) >= number_non_optional_descriptors_remaining
